...
### スケジュール個人設定
- **`/setup_schedule` コマンド**: スケジュール通知設定パネルを設置（管理者限定）
- **カテゴリ選択**: メンバーが受け取りたい情報をSelectMenuで選択
  - ナワバリバトル / バンカラマッチ / Xマッチ / サーモンラン
- **DM配信**: 設定した情報のみがDMで届く
...
### スケジュール個人設定
1. 管理者が `/setup_schedule` コマンドで設定パネルを設置
2. メンバーはSelectMenuで受け取りたいカテゴリを選択
3. 選択したカテゴリの情報がDMで届きます
...
│   ├── autoSchedule.js     # スケジュール自動送信タイマー（削除・DM対応）
│   └── schedulePrefs.js    # スケジュール個人設定パネル
